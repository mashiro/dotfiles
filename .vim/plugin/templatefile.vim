...
function! LoadTemplateFile()
	...
	let inc_guard  = Escape(substitute(myfile, "\\.", "_", "g"))
	let inc_guard  = Escape(toupper(inc_guard))
	if exists("g:author")
	...
	" nepto@platon.sk    --> #.platon.sk --> sk.platon.@INCLUDE_guard@
	" rajo AT platon.sk  --> #.platon.sk  --> sk.platon.@INCLUDE_guard@
	...
	endwhile
	let java_pkg = substitute(java_pkg, '^\.\(.*\)#$', '\1.' . tolower(inc_guard), '')
