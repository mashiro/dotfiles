...
/**
 * YouTube HD Suite Object
 */
w.hdsuite = {
    /**
     * YouTube HD Suite version
     */
    version: { text:'2.0.3', rev:200911240030 },
    /**
     * Auto version check
     */
    ...
    createMenu: function() {
        ...
        for ( var i in w.hdsuite.config.download_formats ) {
            ...
            html
                ...
                +  '<td style="'+w.hdsuite.tx.mono+'">'+w.hdsuite.formats[i].desc+'</td>'
                +  '</tr>';
        }
    ...
    checkURL: function(video_id) {
        ...
        XHR.onreadystatechange = function() { 
            if ( XHR.readyState==4 && (match = XHR.responseText.match(/'SWF_ARGS': ({.*})/)) ) {
                var json = eval('('+RegExp.$1+')');
                var map = decodeURIComponent(json['fmt_url_map']);
                var strong = d.getElementsByTagName('strong');
                for ( var i=0; i<strong.length; i++ ) {
                    if ( !strong[i].innerHTML && strong[i].getAttribute('class') == json['video_id'] ) {
                        ...
                        for ( var j in w.hdsuite.config.download_formats ) {
                            ...
                            if ( flag && (
                                ...
                                link.href      = '/get_video?fmt='+j+'&video_id='+json['video_id']+'&t='+json['t'];
    ...
    visualize: function() {
        ...
        for ( var i=0; i<a.length; i++ ) {
            match = '';
            if ( a[i].innerHTML.match(/<img/i) )             continue; // Skip Image Link
    ...
    checkFormat: function(fmt) {
        // Local check
        var map = decodeURIComponent(s['fmt_url_map']);
        if ( map.indexOf(fmt+'|') > -1 ) {
//console.log(fmt);
            d.getElementById('check_fmt_'+fmt).innerHTML
                = '<a href="'+w.hdsuite.makeDownloadURL(fmt)+'" style="'+w.hdsuite.tx.dl+'background:#'+w.hdsuite.formats[fmt].color+';border:1px solid #'+w.hdsuite.formats[fmt].color+';">[OK] '+w.hdsuite.formats[fmt].desc+'</a>';
            return true;
        }
        ...
        XHR.onreadystatechange = function() { 
            if (XHR.readyState==4) {
                ...
                if ( match = XHR.responseText.match(/'SWF_ARGS': ({.*})/) ) {
                    ...
                    if ( map.indexOf(fmt+'|') > -1 ) {
                        block.innerHTML
                            += '<a href="'+w.hdsuite.makeDownloadURL(fmt)+'" style="'+w.hdsuite.tx.dl+'background:#'+w.hdsuite.formats[fmt].color+';border:1px solid #'+w.hdsuite.formats[fmt].color+';">'
                            +  '[OK] '+w.hdsuite.formats[fmt].desc+'</a>';
                    } else {
                        block.innerHTML
                            += '<span  style="'+w.hdsuite.tx.mono+'">[NG] '+w.hdsuite.formats[fmt].desc+'</span>';
                    }
                }
            }
        }
...
*/
    ...
    showDownloadLinks: function() {
        if( !d.getElementById('hdsuite_dl_div') ) {
            // Create Links Block
            d.getElementById('watch-embed-div').innerHTML += '<div id="hdsuite_dl_div" style="margin:10px 0;"></div>';
        }
        for ( var i in w.hdsuite.config.download_formats ) {
            fmt    = w.hdsuite.config.download_formats[i];
            if ( !fmt ) continue;
            ...
            if ( config.check ) {
                d.getElementById('hdsuite_dl_div').innerHTML
                    += '<div id="check_fmt_'+i+'">checking fmt='+i+'</div>';
                w.hdsuite.checkFormat(i);
            } else {
                d.getElementById('hdsuite_dl_div').innerHTML
                    += '<div><a href="'+w.hdsuite.makeDownloadURL(i)+'" style="'+w.hdsuite.tx.dl+'background:#'+w.hdsuite.formats[i].color+';">[OK] '+config.desc+'</a></div>';
            }
        }
    },
    sprite:
     ...
     */
     tx: {
        mono: 'display:block;font:12px/12px Osaka-Mono,\'MS Gothic\',Monaco,monospace;white-space:pre;padding:3px;',
